<template>
  <div :class="['rounded-lg p-4 w-80', column.color]">
    ...
    >
      <template #item="{ element }">
        <TaskCard :task="element" @remove="handleRemoveTask" />
      </template>
    </draggable>

    <AddTaskForm v-if="column.id !== 'done'" :column="column" @add="handleAddTask" />
  </div>
</template>
