...
export const useBoardStore = defineStore('board', () => {
  ...
  function removeColumn(id) {
    ...
    if (confirm(`Are you sure you want to delete column "${columnName}"?`)) {
      if (numTasks > 0) {
        if (
          confirm(
            `There are ${numTasks} tasks in column "${columnName}". Would you like to mark all of them as done?`,
          )
        ) {
  ...
  function removeTask(taskId) {
    if (
      confirm(
        `Are you sure you want to delete task "${tasks.value.find((t) => t.id === taskId).title}"?`,
      )
    ) {
